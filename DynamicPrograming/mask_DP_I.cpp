	...
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < m; j++) {
			...
			for(int k = 0, s; s = status[k], s < bit3[m + 1]; k++) {
				...
				} else if(l == 1 && u == 2) {/* only happen last step*/}
			}
		}
		swap(pre, now); CC(dp[now], 0);//memset to set illegal state -1
