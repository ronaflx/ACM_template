...
public:
	splay_tree() {
		root = null = new node(node::INF, 0, 0, 0, 0);
	}
	~splay_tree() {
		clear(root);
		delete null;
	}
	// build(0, n + 1, val) make a sequence from 1 to n
	void build(int l, int r, int* val) {
		if(l > r) return;// and make sure val[0] = va[1] = INF;
...
#define centre (root->ch[1]->ch[0])
	...
	void erase(int a) {
		...
		delete centre;
		centre = null;
		root->ch[1]->push_up(), root->ch[0]->push_up();
	}
#undef centre
