...
int _rank[MAXN], bucket[MAXN], height[MAXN];// _rank from 0 to n - 1
...
void buildSA(const char* text, int n) {/*text can't contain 0, 0 is used as terminal*/
	for (int i = 0; i < n; i++)
	...
	while (wid < n) {
		/*get current rank*/
		_rank[sfx[0]] = 0; 
		for (int i = 1; i < n; i++) _rank[sfx[i]] = _rank[sfx[i - 1]] + cmp(i - 1, i);
		/*sort current suffix, radixSort，first key[1] then key[0]*/
		for (int i = 0; i < n; i++) {
			sfx[i] = i;
		...
		/*recalculate key, used by rank*/
		for(int i = 0;i < n;i++) key[i][0] = _rank[sfx[i]];
		for(int i = 0;i < n;i++) key[i][1] = wid + sfx[i] < n ? _rank[sfx[i] + wid] : 0;
		wid <<= 1;
	}
}
...
int RMQ[MAXN][20];
void buildRMQ(int n, int* height) {//n = len(text),height[0] means nothing
